...
def nanocompore_postprocess(
    input_tsv, fasta, outdir, log, p_val_lim=0.01, quantile_lim=0.5, min_distance=9
):

    logging.basicConfig(
        filename=log, filemode="w", level=logging.INFO, format="%(message)s"
    )
    ...
    df = pd.read_csv(input_tsv, sep="\t", dtype={"chr": str}, na_values=["NC"])
    ...
    for test in list(tests_d.keys()):
        ...
        with open(
            os.path.join(outdir, f"nanocompore_results_{test_label}.tsv"), "w"
        ) as res_fp:
            res_fp.write("ref_id\tpos\tpvalue\tpeak\tref_kmer\tLOR\n")
            # Extract data for current test and cleanup
            test_df = df[["ref_id", "pos", test, "ref_kmer", "Logit_LOR"]].copy()
            test_df = test_df.rename(columns={test: "pvalue", "Logit_LOR": "LOR"})
            ...
            for tx, tx_df in test_df.groupby("ref_id"):
                c["All transcripts"] += 1
                x = pd.Series(
                    data=-np.log10(tx_df["pvalue"]).values, index=tx_df["pos"].values
                )
                    ...
                    for i in tx_df.itertuples():
                        if i.pvalue <= p_val_lim:
                            ...
                            if i.pos in peaks:
                                ...
                                peak = False
                            res_fp.write(
                                f"{i.ref_id}\t{i.pos}\t{i.pvalue}\t{peak}\t{i.ref_kmer}\t{i.LOR}\n"
                            )
